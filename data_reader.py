...
class NerDataReader:
    ...
    def post_process(self, exm: NerExample, lang='ENG', train=True, arch='seq', loss_type='sigmoid'):
        if not hasattr(exm, 'train_cache'):
            ...
            if lang in ['ENG', 'ZHENG']:
                exm.train_cache.update(ori_len=len(exm.char_lst))
                exm.train_cache.update(ori_indexes=exm.ori_indexes)
            if train:
                if arch == 'seq':
                    ...
                    link_lst = exm.get_link_lst()
                    exm.train_cache.update(link_lst=link_lst)
    ...
    def get_batcher_fn(self, gpu=False, device=None, arch='span'):
        ...
        def span_batcher(batch_e):
            ...
            if 'ori_indexes' in batch_e[0]:  # ENG
                batch_ori_indexes = [tensorize(ex['ori_indexes']) for ex in batch_e]  # e.g. [0, 1, 1, 1, 2, 3, 4, 5, 6, 7, 8]
                batch_ori_indexes = torch.nn.utils.rnn.pad_sequence(batch_ori_indexes, batch_first=True, padding_value=-1)
            else:
                batch_ori_indexes = None
            ...
            batch_link_lst = torch.nn.utils.rnn.pad_sequence([tensorize(e['link_lst']) for e in batch_e], batch_first=True, padding_value=0.)
            ...
            return {
                ...
                'batch_ori_indexes': batch_ori_indexes,
    ...
    def build_dataset(self, data_source, lang='ENG', arch='span', loss_type=None,
        ...
        if lang == 'ENG':  # generate sub_tokens and ori_indexes
            for exm in exm_lst:
                if not hasattr(exm, 'sub_tokens'):
                    exm.process_EN_by_tokenizer(self.tokenizer)
        elif lang == 'ZHENG':
            for exm in exm_lst:
                if not hasattr(exm, 'sub_tokens'):
                    exm.process_ZHENG_by_tokenizer(self.tokenizer)
        else:
            pass

        # auto segment exm if exceed over max_len
        segmented_nums = 0
